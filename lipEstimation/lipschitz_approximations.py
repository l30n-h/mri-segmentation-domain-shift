...
def lipschitz_spectral_ub(model, requires_grad=False):
    ...
    for p in model.modules():
        if hasattr(p, 'spectral_norm'):
            # print(p.__class__.__name__, p.spectral_norm, lipschitz)
...
def lipschitz_frobenius_ub(model, requires_grad=False):
    ...
    def prod_frob(self, input, output):
        ...
        if len(ps) == 0:
            return
        self.frob_norm = { n: torch.norm(p, p=2).cpu() for n, p in ps }
    execute_through_model(prod_frob, model, backwards=False)
    #res = lip
    #del lip
    #return res
    ...
    for p in model.modules():
        if hasattr(p, 'frob_norm'):
            # print(p.__class__.__name__, p.frob_norm, frob)
            frob = frob * p.frob_norm['weight']  # not in-place for grad
    return frob
...
def lipschitz_second_order_ub(model, algo='greedy'):
    ...
    def affine_layers_aux(self, input, output):
        ...
        if not is_convolution_or_linear(self):
            return
        ps = list(self.parameters(recurse=False))
        if len(ps) == 0:
            return
        p = ps[0]
